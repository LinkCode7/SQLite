...
namespace Sindy
{
	...
	VirtualItem::VirtualItem(const std::string& strKey, int iType) :
		m_strKey(strKey),
		m_iType(iType)
	{}
	...
	ColumnItem::ColumnItem(const std::string& strName, const std::string& strCaption, bool isFixed, int iWidth, bool isHide) :
		VirtualItem(strName, kTextField),
		m_strValue(strCaption),
		m_isFixed(isFixed),
		m_iWidth(iWidth),
		m_isHide(isHide)
	{
	...
	TableData::TableData() : m_strDbAliasName(_T("main"))
	{
	}
	...
	{
		//_T("update table set show_id=:1 where com_id=:2")
	...
	{
		strCreateSql = std::format(_T("CREATE TABLE {}.'{}'("), m_strDbAliasName, strTableName);
	...
	{
		std::string strCreateSql = std::format(_T("CREATE TABLE {}.'{}'("), m_strDbAliasName, strTableName);
	...
	{
		return;
		std::string strSelect = _T("select ");
	...
	TableDataPtr Dataset::GetItem(const std::string& strRowName)
	{
		...
		{
			if (data->GetValueString(_T("name")) == strRowName)
				return data;
		}
		return TableDataPtr();
	}
	...
	Dataset2::Dataset2(const TableDataPtr& schema) :
		m_schema(schema)
	{
