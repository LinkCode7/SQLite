...
namespace SQLiteCFunction
{
	...
	int testInsert()
	{
...

		{
			...
			{
				char pSql[1000];
				// std::sprintf(pSql, "INSERT INTO %s (com_id,com_type,floor_id,name,show_id,time) VALUES (%d, %d, 1, 'Paul', 100, datetime('now','localtime'))", COMPONENT_TABLE_NAME, i, i*1000);
				std::sprintf(pSql, "INSERT INTO %s (com_id,com_type,floor_id,name,show_id) VALUES (%d, %d, 1, 'Paul', 100)", COMPONENT_TABLE_NAME, i, i * 1000);
