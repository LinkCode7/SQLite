...
namespace TestMathExpresstion
{
	...
	void testCalculation(const std::map<std::string, double>& mapExpresstion2Value)
	{
		...
		for (; iter != mapExpresstion2Value.end(); ++iter)
		{
			...
			if (Sindy::compareDouble(dRightValue, dValue) != 0)
				std::cout << "Calculate Error" << std::endl;
		}
	}

	void muParserTest(const std::map<std::string, double>& mapExpresstion2Value)
	{
#ifdef _MU_PARSER
		using namespace mu;
		Parser muparser;
		...
		for (; iter != mapExpresstion2Value.end(); ++iter)
		{
			...
			muparser.SetExpr(iter->first);
			double dValue = muparser.Eval();
