...
bool ExpresstionEval::CheckNextWord(int place)const
{
	...
	case kMathLevel_1: // 当前字符是数字时
	{
		if (place == m_length - 1) // 最后的字符是数字时
			return true;
		else if (m_arrTag[place + 1] == kMathLevel_1 || m_arrTag[place + 1] == kMathLevel_2 || m_arrTag[place + 1] == kMathLevel_4 || m_arrTag[place + 1] == kMathLevel_5)
			return true;
		return false;
	}
	...
	case kMathLevel_5: // 当前字符是'.'号时
	{
		if (place == m_length - 1)
			return true;
		else if (m_arrTag[place + 1] == kMathLevel_2 || m_arrTag[place + 1] == kMathLevel_1 || m_arrTag[place + 1] == kMathLevel_4) //运算符，数字，右括号
			return true;
		return false;
	}
	default:
...
void ExpresstionEval::Extraction()
{
	...
	for (int i = 0; i < m_length; ++i)
	{
		...
		// 如果当前是字符'-'，且表示负
		else if ((m_arrTag[i - 1] == kMathLevel_2 && m_strExp[i] == '-') ||
			(i == 0 && m_strExp[i] == '-') ||
